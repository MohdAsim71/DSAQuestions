...
fun removeDuplicates(head: com.example.dsaquestions.questions.linkedlist.Node?): com.example.dsaquestions.questions.linkedlist.Node? {
    var current = head
...
fun printList(head: com.example.dsaquestions.questions.linkedlist.Node?) {
    var current = head
    while (current != null) {
        print("${current.data} -> ")
        current = current.next
